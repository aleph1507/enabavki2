...
try {
	var pid = npid.create('/var/www/phantom/enabavki_test/enabavki.pid', 'force');
	pid.removeOnExit();
} catch(err) {
	console.log(err);
}
  ...
  child.stdout.on('data', function(data) {
    ...
    if(count == 2){
	  ...
	  send = true;
		...
		console.log('read[k].split( -|- )[0]: ', read[k].split(' -|- ')[0]);
		console.log('dataArr[i].split( -|- )[0]: ', dataArr[i].split(' -|- ')[0]);
               if(read[k].split(' -|- ')[0] == dataArr[i].split(' -|- ')[0])
                  send = false;
              }
	     console.log('pred if, send: ', send);
              if(send){
		console.log('vo if(send)');
                ...
                read.push(dataArr[i]);
		//sentMails.push(mailData[0]);
		mailID = mailData[0];
		
		console.log('pred for, doSend: ', doSend);
		for(let i0 = 0; i0<sentMails.length; i0++) {
			console.log('vo for, doSend = ', doSend);
			console.log('sentMails[', i0, ']: ', sentMails[i0]);
			if(mailData[0] == sentMails[i0]){
				console.log('mailData[0] = sentMails[', i0, '] = ', sentMails[i0]);
				doSend = false;
			}
		}
                var mailOptions = {
		...
		console.log('pred doSend: ', doSend);
		console.log('pred doSend, mailText:', mailText);
		if(doSend){
		  console.log('vo doSend');
